...
def extract_ot_features(coupling, cross_matrix, C1, C2):
    ...
    # For each matched pair, compare their degrees/connectivity
    structural_mismatch = 0.0
    total_mass = 0.0
    for i in range(n):
        for j in range(m):
            if coupling[i, j] > eps:
                # Compare degrees of matched nodes (weighted by coupling)
                degree_diff = abs(C1[i, :].sum() - C2[j, :].sum())
                structural_mismatch += coupling[i, j] * degree_diff
                total_mass += coupling[i, j]
    
    if total_mass > 0:
        structural_mismatch /= total_mass
...
'''
def make_simulation(n_sample=150, output_dir='risultati'):
    ...
    for dataset in dataset_names:
        ...
        if os.path.exists(dataset_path_true_ged):
            ...
            sampled_true_ged_df = true_ged_df.sample(n=n_sample)
        else:
        ...
        for index, row in sampled_true_ged_df.iterrows():
            ...
            if dataset == 'AIDS':
                ...
                if labels_1 is not None and len(labels_1) != G_1.number_of_nodes():
                    raise ValueError(f"Length of L1 ({len(labels_1)}) does not match the number of nodes in G_1 ({G_1.number_of_nodes()}).")
                if labels_2 is not None and len(labels_2) != G_2.number_of_nodes():
                    raise ValueError(f"Length of L2 ({len(labels_2)}) does not match the number of nodes in G_2 ({G_2.number_of_nodes()}).")
                ...
                structural_cross_matrix = compute_cross_matrix_with_structural_features(G_1, G_2, list_of_centrality_indices=None)
                ...
                gw_score = compute_ged_GW(G_1, G_2, cross_matrix_no_structural_features)
                results_no_structural_features = pd.concat([results_no_structural_features, pd.DataFrame({'GW_Score': [gw_score], 'True_GED': [true_ged]})], ignore_index=True)
                ...
                gw_score = compute_ged_GW(G_1, G_2, cross_matrix_with_structural_features)
                results_with_structural_features = pd.concat([results_with_structural_features, pd.DataFrame({'GW_Score': [gw_score], 'True_GED': [true_ged]})], ignore_index=True)
                ...
                gw_score = compute_ged_GW(G_1, G_2, cross_matrix_with_structural_features)
                results_with_structural_features = pd.concat([results_with_structural_features, pd.DataFrame({'GW_Score': [gw_score], 'True_GED': [true_ged]})], ignore_index=True)
        # Save results to CSV files in the output_dir folder
        os.makedirs(output_dir, exist_ok=True)
        results_no_structural_features.to_csv(os.path.join(output_dir, f'{dataset}_no_structural_features.csv'), index=False)
        results_with_structural_features.to_csv(os.path.join(output_dir, f'{dataset}_with_structural_features.csv'), index=False)

make_simulation()

# Apply GED_prediction_regression to each dataset result in 'risultati'
result_files = [f for f in os.listdir('risultati') if f.endswith('.csv')]
for file in result_files:
    file_path = os.path.join('risultati', file)
    print(10*'*')
    print(f"Running regression for {file_path}")
    GED_prediction_regression(file_path)
